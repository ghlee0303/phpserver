...
function unspec_table_temp_set($user_index, $user_name, $user_id, $user_pwd, $user_phone, $user_email)
{
...
function spec_table_set($data_list)
{
  ...
  foreach ($data_list as $index => $value) {
    $spec_table_set = $spec_table_set . spec_table_temp_set($value["id"], $value["name"], $value["user_id"], "*****", $value["phone"], $value["email"], $value["branch"]);
  }
...
function unspec_table_init($mysqli, &$unspec_list)
{
  ...
  foreach ($unspec_list as $index => $value) {
    $unspec_table_set = $unspec_table_set . unspec_table_temp_set($value["id"], $value["name"], $value["user_id"], "*****", $value["phone"], $value["email"]);
  }
...
{
  $user_sql = "SELECT * FROM user WHERE not position in('99') OR position is null";          // 미할당
...
<body>
  ...
  <div class=" container container-mobile-1 pb-3">
    ...
    <div class="row mt-5 d-flex d-none auth">
      <div id="unspec_install" class="col-44 p-0">
      ...
      <div id="spec_install" class="col-44 p-0">
        <div class="flex-center fs-4 h-4r border-bl mb-3">
          <div class="dropdown text-center">
...
<script>
  ...
  window.onload = function() {
    ...
    $(".branch").each(function(index, branch) {
      dropdown_value($(this).find("button"), $(this).attr("value"));
    });
    ...
    $(".test2").on("click", function() {
      console.log($(".installer").siblings(".dropdownMenu").attr("value"));
    });
